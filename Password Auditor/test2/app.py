...
def load_credentials_from_json(file):
    """
    Loads credentials from a JSON file-like object.
    """
    try:
        file.seek(0)  # Ensure we read from the start of the file-like object
        data = json.load(file)
        credentials = [(item['login']['username'], item['login']['password']) for item in data['items']]
    except KeyError:
        raise ValueError("JSON file is missing 'items', 'login', or 'username'/'password' fields.")
...
def upload_file():
    """
    Uploads a JSON file and starts the login testing process.
    """
    ...
    try:
        credentials = load_credentials_from_json(file)
